...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (name && role && userId) {
      let email: string | undefined = `${name.toLowerCase().replace(/[^a-z0-9.]/g, "").split(" ").join(".")}@campus.edu`;
  ...
  const userInitials = userInfo.name.split(' ').map(n => n[0]).join('').toUpperCase() || (userInfo.email ? userInfo.email[0].toUpperCase() : 'U');
